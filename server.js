...
app.use(cors());                       // 前后端分离部署时按需限制 origin
...
app.get('/api/catalog', authRequired, wrap(async (req, res) => {
  const [chapters, sections, chAgg, secAgg, papers] = await Promise.all([
    Q.chapters(), Q.allSections(), Q.countAllChapters(), Q.countAllSections(), Q.distinctPapers(),
  ]);
  ...
  res.json({ meta: { total, mc: out.reduce((a, c) => a + c.mc, 0), tf: out.reduce((a, c) => a + c.tf, 0),
             papers: papers.map(r => r.paper) }, chapters: out });
}));
...
app.get('/api/search', authRequired, wrap(async (req, res) => {
  ...
  const rows = await Q.search(like);
...
app.post('/api/practice/start', authRequired, wrap(async (req, res) => {
  const { mode = 'random', id, count = 10 } = req.body || {};
...
app.get('/api/progress', authRequired, wrap(async (req, res) => {
  ...
  const [progRows, ta, answered, wbc, bms, chapters, chAgg] = await Promise.all([
    Q.progressByChapter(uid), Q.totalAttempts(uid), Q.totalAnswered(uid),
    Q.wrongbookCount(uid), Q.bookmarkQids(uid), Q.chapters(), Q.countAllChapters(),
  ]);
  ...
  res.json({
    answered: N(answered.c), total_questions: by_chapter.reduce((a, c) => a + c.total, 0),
...
(async () => {
  await initDb();
  app.listen(PORT, () => console.log(`\n✅ GESP 学习平台已启动: http://localhost:${PORT}\n`));
